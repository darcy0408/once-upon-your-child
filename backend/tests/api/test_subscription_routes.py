...
def test_get_subscription_success_contract(client, app):
    ...
    response = client.get(
        f"/api/user/{user_id}/subscription", headers=_auth_headers(user_id)
    )
...
def test_get_subscription_user_not_found_returns_404(client, auth_token):
    ...
    response = client.get(
        "/api/user/missing-user-id/subscription",
...
def test_get_subscription_returns_500_on_unexpected_error(client, mocker, app):
    ...
    mocker.patch(
        "backend.routes.subscription_routes.db.session.get",
        side_effect=RuntimeError("db down"),
    )

    response = client.get(
        "/api/user/sub-user-err/subscription", headers=_auth_headers("sub-user-err")
    )
