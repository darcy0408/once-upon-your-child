...
def test_get_overview_success(client, admin_headers, app):
    ...
    with app.app_context():
        # Add some data
        user = User(id=str(uuid.uuid4()), username='u1', email='u1@e.com', role='user', password_hash='hash')
        db.session.add(user)
        db.session.commit()
        
        story = Story(id=str(uuid.uuid4()), user_id=user.id, title='T1', theme='Adventure')
...
def test_get_stories_paginated_success(client, admin_headers, app):
    ...
    with app.app_context():
        user = User(id=str(uuid.uuid4()), username='u2', email='u2@e.com', password_hash='hash')
        db.session.add(user)
        db.session.commit()
        story = Story(id=str(uuid.uuid4()), user_id=user.id, title='P1', theme='Fantasy')
        db.session.add(story)
        db.session.commit()

    response = client.get('/admin/analytics/stories?page=1&per_page=10', headers=admin_headers)
    assert response.status_code == 200
    data = response.get_json()
