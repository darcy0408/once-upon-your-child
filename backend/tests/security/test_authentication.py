...
def mock_auth_secret(mocker):
    ...
    return mocker.patch(
        "backend.middleware.auth._get_jwt_secret", return_value="dev-secret-key"
    )
...
def test_require_admin_success(app, client):
    ...
    with app.app_context():
        admin = User(
            id=admin_id, username="admin_test", email="admin@test.com", role="admin"
        )
...
def test_token_validation_edge_cases(app, client, auth_headers):
    ...
    response = client.get(
        "/test/auth", headers={"Authorization": f"Bearer {ghost_token}"}
    )
    assert response.status_code == 401
    assert response.json["error"] == "User not found"
