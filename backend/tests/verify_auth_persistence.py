    ...
    with app.test_client() as client:
        ...
        with app.app_context():
            ...
            if resume_data["user_id"] != user_id:
                print(
                    f"❌ Warning: Resumed user_id {resume_data['user_id']} does not match original {user_id}"
                )
            else:
            ...
            resp = client.get(
                "/get-characters", headers={"Authorization": f"Bearer {resume_token}"}
            )
            ...
            if resp.status_code == 200:
                ...
                if found:
                    print(
                        f"✅ SUCCESS: Character {CHARACTER_ID} persisted and retrieved."
                    )
                else:
