...
class TestAvatarGenerationService:
    def test_generate_avatar_rejects_invalid_inputs(self):
        service = AvatarGenerationService(
            image_generator=MagicMock(), fallback_generator=MagicMock()
        )
    ...
    def test_generate_avatar_success(self):
        ...
        service._generate_image_with_gemini = MagicMock(
        ...
        assert result["image_base64"] == "data:image/png;base64," + base64.b64encode(
            b"fake_image_bytes"
        ).decode("utf-8")
    ...
    def test_generate_image_with_gemini_works(self):
        primary = MagicMock()
        ...
        service = AvatarGenerationService(
            image_generator=primary, fallback_generator=fallback
        )
