...
def test_get_subscription_success(client, setup_users):
    ...
    response = client.get(
        f"/api/user/{user_id}/subscription", headers=_get_auth_headers(user_id)
    )
...
def test_get_subscription_user_not_found(client, setup_users):
    ...
    with client.application.app_context():
        caller_id = _create_user(
            id="caller-1", username="caller", email="caller@test.com"
        )

    # Attempting to access OTHER user ID results in 403 Forbidden due to @require_owner
    response = client.get(
        "/api/user/does-not-exist/subscription", headers=_get_auth_headers(caller_id)
    )
