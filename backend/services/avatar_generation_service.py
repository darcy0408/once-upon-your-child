...
"""
    ...
    def generate_avatar(
        ...
        if style.lower() not in ["pixar", "watercolor", "cartoon", "clay"]:
            raise ValueError(
                f"Invalid style: {style}. Must be pixar|watercolor|cartoon|clay"
            )

        if self.image_generator is None and self.fallback_generator is None:
            raise Exception(
                "No image generator available (neither Gemini nor OpenRouter)"
            )
        ...
        if seed is None:
            seed = self.prompt_service.generate_character_seed(
                character_name, age, features
            )

        logger.info(
            f"Generating avatar for {character_name}, age {age}, style {style}, seed {seed}"
        )
        ...
        for attempt in range(MAX_RETRIES):
            try:
                image_data = self._generate_image_with_gemini(
                    prompt=prompt, character_name=character_name, age=age, style=style
                )

                # Verify non-photorealistic
                if self._verify_non_photorealistic(image_data):
                    logger.info(
                        f"Avatar passed vision verification on attempt {attempt + 1}"
                    )
                    break  # Success!
                else:
                    if attempt < MAX_RETRIES - 1:
                        ...
                        logger.error(
                            f"Avatar failed vision verification after {MAX_RETRIES} attempts"
                        )
                        raise Exception(
                ...
                if attempt < MAX_RETRIES - 1:
                    logger.warning(
                        f"Avatar generation attempt {attempt + 1}/{MAX_RETRIES} failed: {e}, retrying..."
                    )
                else:
                    logger.error(
                        f"Avatar generation failed after {MAX_RETRIES} attempts: {e}"
                    )
        ...
        logger.info(
            f"Avatar generated successfully: {avatar_id} in {generation_time_ms}ms"
        )
    ...
    def regenerate_avatar(
        ...
        raise NotImplementedError(
            "Regeneration requires stored character data - use generate_avatar with seed parameter"
        )

    def _generate_image_with_gemini(
        self, prompt: str, character_name: str, age: int, style: str
    ) -> bytes:
        """
        ...
        if self.fallback_generator is not None:
            try:
                ...
                if gemini_error:
                    raise Exception(
                        f"Avatar generation failed: Gemini: {str(gemini_error)}, OpenRouter: {str(e)}"
                    )
                else:
                    raise Exception(
                        f"Avatar generation failed (OpenRouter only): {str(e)}"
                    )
    ...
    def _verify_non_photorealistic(self, image_data: bytes) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Avatar vision verification failed due to analyzer error: %s", e
            )
            return False
