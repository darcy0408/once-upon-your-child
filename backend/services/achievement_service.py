...
class AchievementService:
    ...
    def _check_achievement_unlocks(self, user_id: str, stats: AchievementStats) -> list:
        ...
        story_achievements = [
            ('firstStory', 1), ('fiveStories', 5), ('tenStories', 10),
            ('twentyFiveStories', 25), ('fiftyStories', 50), ('hundredStories', 100)
        ]

        for achievement_type, target in story_achievements:
            if stats.total_stories >= target:
                if self._unlock_achievement_if_needed(user_id, achievement_type, stats.total_stories, target):
        ...
        for achievement_type, theme in theme_achievements:
            ...
            if theme_count >= 10:  # All theme achievements require 10 stories
                if self._unlock_achievement_if_needed(user_id, achievement_type, theme_count, 10):
                    new_unlocks.append(achievement_type)

        # Check streak achievements
        streak_achievements = [
            ('streak3', 3), ('streak7', 7), ('streak30', 30)
        ]

        for achievement_type, target in streak_achievements:
            if stats.longest_streak >= target:
                if self._unlock_achievement_if_needed(user_id, achievement_type, stats.longest_streak, target):
                    new_unlocks.append(achievement_type)

        # Check character achievements
        char_achievements = [
            ('characterCreator', 3), ('characterChampion', 10)
        ]

        for achievement_type, target in char_achievements:
            if stats.characters_created >= target:
                if self._unlock_achievement_if_needed(user_id, achievement_type, stats.characters_created, target):
                    new_unlocks.append(achievement_type)

        return new_unlocks

    def _unlock_achievement_if_needed(self, user_id: str, achievement_type: str, current_value: int, target_value: int) -> bool:
