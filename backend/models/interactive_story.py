...
class InteractiveStory(db.Model):
    ...
    )  # whimsical, mystery, sci-fi, fantasy, cozy-adventure
    ...
    world_bible = db.Column(
        db.Text, nullable=True
    )  # Rich world description for AI consistency

    # Progress tracking
    current_segment_id = db.Column(
        db.String(36),
        db.ForeignKey(
            "story_segment.id", use_alter=True, name="fk_story_current_segment"
        ),
        nullable=True,
    )
    ...
    inventory = db.relationship(
        "InventoryItem", backref="story", lazy="dynamic", cascade="all, delete-orphan"
    )
    state = db.relationship(
        "StoryState", backref="story", uselist=False, cascade="all, delete-orphan"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "inventory": [
                item.to_dict()
                for item in self.inventory.filter_by(is_active=True).all()
            ],
...
class StorySegment(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    story_id = db.Column(
        db.String(36), db.ForeignKey("interactive_story.id"), nullable=False, index=True
    )

    # Segment metadata
    segment_number = db.Column(
        db.Integer, nullable=False
    )  # Sequential number (1, 2, 3...)
    title = db.Column(db.String(200), nullable=True)  # Optional segment title
    stage_label = db.Column(
        db.String(100), nullable=True
    )  # Kid-friendly stage label (e.g., "Play Time!")
    ...
    )  # Actual word count for pacing tracking

    # Content
    content = db.Column(
        db.Text, nullable=False
    )  # Story prose (now 350-650 words for immersion)
    image_description = db.Column(
        db.Text, nullable=True
    )  # Description for illustration generation
    image_url = db.Column(db.Text, nullable=True)  # Base64 data URI or URL

    # Navigation
    parent_choice_id = db.Column(
        db.String(36),
        db.ForeignKey(
            "story_choice.id", use_alter=True, name="fk_segment_parent_choice"
        ),
        nullable=True,
    )
...
class StoryChoice(db.Model):
    ...
    segment_id = db.Column(
        db.String(36), db.ForeignKey("story_segment.id"), nullable=False, index=True
    )

    # Choice metadata
    choice_number = db.Column(
        db.Integer, nullable=False
    )  # 1-4 depending on story length
    text = db.Column(
        db.String(500), nullable=False
    )  # The choice text displayed to user
    consequence_type = db.Column(
        db.String(100), nullable=True
    )  # location_change, item_gained, clue_found, etc.
...
class StoryState(db.Model):
    ...
    story_id = db.Column(
        db.String(36),
        db.ForeignKey("interactive_story.id"),
        nullable=False,
        unique=True,
        index=True,
    )
