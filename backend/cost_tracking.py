...
def track_cost(
    ...
    # Log the cost event
    logging.info(
        f"Cost tracked: {operation} for user {user_id} ({user_tier}): ${cost:.6f}"
    )
...
def _check_budget_limits():
    ...
    daily_costs = [event.cost for event in _cost_events if event.timestamp >= day_start]
    ...
    weekly_costs = [
        event.cost for event in _cost_events if event.timestamp >= week_start
    ]
...
def get_cost_report(days: int = 7) -> Dict[str, Any]:
    ...
    relevant_events = [event for event in _cost_events if event.timestamp >= cutoff]
    ...
    return {
        ...
        "budget_limits": {"daily": DAILY_BUDGET_LIMIT, "weekly": WEEKLY_BUDGET_LIMIT},
...
def get_daily_total() -> float:
    ...
    today_events = [event for event in _cost_events if event.timestamp.date() == today]
...
def get_weekly_total() -> float:
    ...
    week_events = [event for event in _cost_events if event.timestamp >= week_start]
