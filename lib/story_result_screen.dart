...
class _StoryResultScreenState extends State<StoryResultScreen> {
  ...
  Future<void> _fetchAsyncIllustrations() async {
    if (!mounted) return;
    ...
    try {
      final illustrations = await ApiServiceManager.fetchIllustrationsAsync(
        storyText: widget.storyText,
        characterName: widget.characterName ?? 'the hero',
        age: _effectiveAge,
        numImages: 1, // Start with 1 illustration
      );

      if (mounted && illustrations.isNotEmpty) {
        _updateWithNewIllustrations(illustrations);
      }
    } catch (e) {
      debugPrint('Error fetching async illustrations: $e');
      if (mounted) {
        _showSnackBar('Could not load illustrations automatically.', backgroundColor: Colors.orange);
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  void _updateWithNewIllustrations(List<Map<String, dynamic>> newIllustrations) {
    if (!mounted) return;

    final decoded = <_InlineIllustration>[];
    for (final item in newIllustrations) {
    ...
    setState(() {
      _inlineIllustrations = [..._inlineIllustrations, ...decoded];
    });
    
    if (decoded.isNotEmpty) {
      _showSnackBar('Illustration generated!', backgroundColor: Colors.green);
    }
  }
  ...
  void initState() {
    ...
    // Check if we need to fetch illustrations asynchronously
    if (widget.asyncIllustrations && 
        (widget.backendIllustrations == null || widget.backendIllustrations!.isEmpty)) {
      _fetchAsyncIllustrations();
    }
  }
...
class _StoryResultScreenState extends State<StoryResultScreen> {
  ...
  void initState() {
    ...
    // Check if we need to fetch illustrations asynchronously
    if (widget.asyncIllustrations && (widget.backendIllustrations == null || widget.backendIllustrations!.isEmpty)) {
      _fetchAsyncIllustrations();
    }
  }
  ...
  Future<void> _fetchAsyncIllustrations() async {
    ...
    try {
      ...
      if (mounted && illustrations.isNotEmpty) {
        // Convert map to StoryIllustration objects by saving/caching them
        // First, add to inline illustrations for immediate display
        _updateWithNewIllustrations(illustrations);
      }
    } catch (e) {
      debugPrint('Error fetching async illustrations: $e');
  ...
  void _updateWithNewIllustrations(List<Map<String, dynamic>> illustrations) {
    // Decode and add to _inlineIllustrations
    final decoded = <_InlineIllustration>[];
    ...
    setState(() {
      _inlineIllustrations.addAll(decoded);
    });
