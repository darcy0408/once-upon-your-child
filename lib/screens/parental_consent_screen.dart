...
class _ParentalConsentScreenState extends State<ParentalConsentScreen> {
  ...
  /// The parent has read far enough down the notice to unlock the consent
  /// controls. Drives both the footer's locked-state hint and the bottom-edge
  /// "keep scrolling" cue, so the two can never disagree.
  bool get _readEnough => _scrollProgress >= 0.95;
                        ...
                        Expanded(
                          child: Stack(
                            children: [
                              ...
                              if (!_readEnough) _buildScrollMoreCue(),
                            ],
                          ),
                        ),
                        _buildStickyFooter(),
  ...
  /// Bottom-edge "there is more below" cue for the notice.
  ///
  /// The notice is long, and on a phone the scroll body is clipped flush
  /// against the sticky footer — a paragraph that happens to end near that
  /// seam reads as the end of the document, so parents did not realise they
  /// had to keep scrolling (reported on-device 2026-08-05). Fading the last
  /// strip into the footer colour shows the text continues *underneath* the
  /// footer, and the chevron names the gesture.
  ///
  /// Shown only while [_readEnough] is false, so once the read-gate is
  /// satisfied nothing overlays the closing lines of the notice. Deliberately
  /// static — CMP-6 keeps animated, child-directed cues off the consent step.
  Widget _buildScrollMoreCue() {
    const footerColor = Color(0xFF1E0A3C);
        ...
        child: Container(
          ...
          decoration: const BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [Color(0x001E0A3C), Color(0xE61E0A3C), footerColor],
              stops: [0.0, 0.6, 1.0],
            ),
          ),
          child: Container(
  ...
  Widget _buildStickyFooter() {
    final bool readEnough = _readEnough;
    ...
    return Container(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          // Says why the controls below are locked. The *direction* to scroll
          // is carried by _buildScrollMoreCue() at the bottom of the notice —
          // this line previously used an up-arrow next to the word "scroll",
          // which pointed away from the gesture the parent actually needs.
          if (!readEnough)
            Padding(
              padding: const EdgeInsets.only(bottom: AppSpacing.xs),
              child: Row(
                ...
                children: [
                  ...
                  Flexible(
                    child: Text(
                      'Read to the end to unlock consent',
                      style: GoogleFonts.fredoka(
                        color: const Color(0xFFFFD700),
                        fontSize: 13,
                        fontWeight: FontWeight.w600,
