...
//
// While the user reads a story, generate one illustration per page in the
// background via the server-managed image pipeline. The reader almost
// always spends longer on a page than generation takes to return (~5–10s
// vs 30–60s of read-aloud time), so a serial worker that prefers the page
// after the user's current position keeps every page illustrated by the
// time they swipe.
//
...
class PageIllustrationState {
  ...
  static const idle =
      PageIllustrationState(status: PageIllustrationStatus.idle);
...
class PerPageIllustrationPrefetcher {
  PerPageIllustrationPrefetcher({
    ...
    http.Client? client,
        ...
        _states = List.generate(
          pageTexts.length,
          (_) =>
              ValueNotifier<PageIllustrationState>(PageIllustrationState.idle),
        );
  ...
  /// When true, use the backend's server-managed image generator for page
  /// art. When false the prefetcher stays idle (kept as an explicit switch
  /// so callers can disable per-page art wholesale).
  ...
  /// Null when no metered response has come back yet.
  int? _quotaUsed;
  int? _quotaLimit;
  ...
  /// page when generation can't possibly succeed (server image quota
  /// exhausted, provider down, etc).
  ...
  Future<void> initialize() async {
    ...
    debugPrint(
      'PerPagePrefetcher: allowServerKey=$allowServerKey; '
      'pages=${pageTexts.length}',
    );
    if (!allowServerKey) {
      // Server-key path not enabled — leave every page idle.
      return;
    }
  ...
  Future<void> _generatePage(int pageIndex) async {
    ...
    try {
      ...
      // success and the quota-exceeded path).
