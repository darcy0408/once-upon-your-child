...
STAGING_BASE_URL is set, and it hard-blocks a small denylist of known prod
hosts. Per the audit safety protocol, if no staging environment exists this
capture is blocked on infrastructure — do not substitute prod.
...
def _append_csv_row(segment: str, p50: float, p95: float, p99: float,
                    n: int, environment: str) -> None:
    CSV_PATH.parent.mkdir(parents=True, exist_ok=True)
    new_file = not CSV_PATH.exists()
    with CSV_PATH.open("a", newline="", encoding="utf-8") as f:
        w = csv.writer(f, lineterminator="\n")
        if new_file:
            w.writerow(["Segment", "p50", "p95", "p99", "Sample Size", "Date", "Environment"])
        w.writerow([
            segment,
            f"{p50:.1f}s", f"{p95:.1f}s", f"{p99:.1f}s",
            n, date.today().isoformat(), environment,
        ])
...
def main() -> None:
    ...
    print(f"Mode: {args.mode}  samples={args.samples}  concurrency={args.concurrency}\n")
    ...
    if p95 > 0 and p99 > 2 * p95:
        print(f"NOTE: p99 ({p99:.1f}s) >> p95 ({p95:.1f}s) — investigate the tail "
              "(likely PERF-02 sync->async double-generation) separately from baseline.")

    if not args.no_csv:
        segment = (
            "Story generation - illustrated (4-image)"
            if illustrated else "Story generation - happy path"
        )
