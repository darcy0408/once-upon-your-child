...
|-------|-------|------|--------|-------|------|
| 2026-08-20 | 09:03 | 76b9 | main (PRs #39, #40, #41, #42 ALL MERGED) | **MT-383 batch closed + three live-reported production bugs fixed**: #39 closed all ten MT-383 items (dead sprout saved-story rows; blank 9-12 gender cards; orphaned feelings art from a hyphen/underscore id mismatch; band-calibrated chronicle chapter tone; character-editor dropdown crashes for archetype-role and 18+ characters; Dark Mode toggle deleted as a verified no-op; hero_placeholder asset created) and generated the 8 missing creator scene tiles. Root cause across nearly all of it was FIVE duplicated band-to-content mappings that had drifted apart — each is now a single source of truth. #40 fixed the follow-on: #39 shipped the art but not the gate that uses it, so creator still drew gradient placeholders over artwork already in the bundle (invisible to tests — the fallback is deliberate; only a human on a device could catch it). #41 fixed an interactive-story failure on the reader's FIRST choice — the segment parser called json.loads in strict mode, which rejects the literal newlines story prose carries, so every retry discarded a valid segment (same fix applied to the main single-shot story path). #42 fixed disabled-button contrast measured at 1.14:1 (now 7.7:1) — Pick-a-Path disables every choice while the next segment generates, so choices were unreadable for the whole wait. Also: native shells finally branded "Once Upon YOUR Child" (the iOS icon read "Story Weaver App", Android read the snake_case package name); CLAUDE.md added (the repo had none, so standing rules were invisible to other agents); image generator migrated off the retired Imagen 4.0 to gemini-3.1-flash-image, with substance/weapon exclusions added to the shared prompt after a render put a lit cigarette in art for 13-14s; IOS_TESTFLIGHT_SETUP.md rewritten as a no-Mac Windows runbook with every command executed and round-trip verified. New: MT-404, MT-405 | private |
...
| 2026-09-25 | 08:57 | 78aa | main (PR #68 merged) | Narration voices: the three US voices in the picker were repointed after the owner ranked candidate samples by ear, and the audio cache key now carries the resolved voice so a repointed entry actually reaches listeners. A dependency security fix (PR #66) was merged first to clear the CVE audit. | private |
| 2026-09-24 | 18:41 | 77cd | main | Screen-free bedtime mode: five audited defects fixed and merged (dropped chip taps, pre-consent narration spend, narration retry, two dead ends, a11y and copy polish). Consent notice now names the LLC and a business phone instead of personal details. | private |
| 2026-09-16 | 08:44 | 9a6e | main (PRs #58, #65 merged) | Pick-a-Path comprehension: ages 8-10 gained a read-aloud vocabulary rule and the leaked "Moment of Wonder" prompt heading is gone. The parental math-gate dialog now paints its own card and ink, fixing invisible copy in all six bands. | private |
| 2026-09-16 | 07:49 | e6d9 | main | Session open and orientation only: health gates and CI checked green, no code changed. Flagged one backlog line still marked open although its fix is already merged. | private |
| 2026-09-15 | 14:36 | a7b9 | main (PR #61 merged) | MT-424 closed: the Easy Reader orb now says "Read-along rhymes", and the end-of-story art upsell no longer advertises the retired bring-your-own-key offer. Both found by rendering the wizard and reader at 360x740 against three real generated stories; four defects that pass came out as MT-438 to MT-441. Also proved the Pick-a-Path "you"-voice rules hold across continuation segments, which had only ever been checked on opening segments. | private |
| 2026-09-14 | 19:08 | 607f | main | Rewrote the Pick-a-Path voice after an adult play-through: every adventure is now told in the "you" voice at any age, the hero's name is said once per segment instead of in every choice, a solo hero no longer gets an invented companion, and adult adventures open by settling the reader into the scene. Readers over 8 now also hear the choices as one spoken question that ends with "or something else?". | private |
| 2026-09-14 | 14:31 | a198 | main | Adversarial six-hats audit of screen-free bedtime mode across the three child bands, parent and child lenses, driven at phone width on the live site. Shipped Limerick Mode as a named Explorer story type (PR #53) and filed MT-430..MT-435 from the audit. | private |
| 2026-09-14 | 14:31 | bd97 | main (PR #54 merged) | A tester could not get past the parental-consent email on TestFlight; the cause was that release iOS builds carried no build-flavor argument, so the app pointed at a development backend on the device itself. Fixed both iOS workflows and the Android runbook, added a test that fails any release build missing the production flavor, shipped a new TestFlight build, and confirmed the whole under-13 consent and story flow on a real Android phone. Filed MT-416 through MT-423 from an owner play-through, plus MT-428 and MT-429 on store-review access and Play internal testing. | private |
| 2026-09-13 | 15:06 | 5229 | main | Reviewed, fixed and merged Limerick Mode (PR #53), then reworked Learning-to-Read for ages 7-8: Easy Reader now writes Dr. Seuss-style couplets while limericks move behind the explicit Limerick Mode choice, with age-graduated humor guidance and a rhyme check that compares sounds instead of spellings (PR #55). | private |
| 2026-09-12 | 18:20 | 8531 | main | **MT-414 + MT-415 closed, MT-411 finished**: local secret-scanning hook installed (plain git hook, not the pre-commit framework — it stashes unstaged changes in a shared checkout) and the one-off full-history scan came back clean; failing Codecov upload removed; model-authored adventure state is now sanitized before it round-trips into the next prompt, and the superhero builders declare the untrusted-input contract (live-probed 0/4 before and after). | private |
| 2026-09-11 | 08:30 | 192d | main | Merged seven CI-verified Dependabot bumps and closed MT-413 — the production smoke tests no longer run on every push (opt-in gate plus a weekly Production Smoke workflow). | private |
| 2026-09-10 | 19:59 | 34fb | main | Fixed an invalid git argument order (`-m` after `--`) in the close-session command template and in the CLAUDE.md rule it was copied from — the documented command could not run as written. Confirmed the iOS TestFlight tester setup is live and working, so MT-404 was closed as already-done. Filed MT-414: the secret-scanning pre-commit hook this repo declares has never actually been installed. | private |
| 2026-09-09 | 17:21 | 2107 | main | Committed the orphaned Stripe subscription work after confirming its rationale was obsolete — anonymous accounts holding gift-code or in-app-purchase entitlement now report the tier they actually hold — and shipped the MT-411 structured-field sanitizer hardening (F2/F4). Also added the missing index row for session b7ce, triaged all ten open Dependabot PRs (seven cleared as single-package and CI-green, three held for local verification), and filed MT-413. | private |
| 2026-09-07 | 15:31 | 3254 | main | Deleted the never-run self-hosted monitoring stack from `monitoring/` (a compose file whose config files were never committed, a Grafana dashboard, and 13 unscheduled scripts) and rewrote `monitoring/README.md` to document only what actually watches production, with a file path per claim. The old README claimed New Relic and Prometheus instrumentation the backend never had. Filed MT-412 for an external uptime probe. | private |
| 2026-08-29 | 07:48 | b7ce | main | Adversarial review of the story-generation input path (`84ffe0b4`, backlog line `c231db22`). Hardened the free-text interactive path in two layers, both needed: the sanitizer now strips to a fix point rather than in a single pass, and the interactive prompts declare an explicit contract that delimited user text is story material and never an instruction. Verified behaviourally against a live model, with every new test proven discriminating by reverting the fix first. Remaining structured-field, round-tripped-state and single-shot items filed as **MT-411**; detail kept in the private record. Row added retroactively on 2026-09-09 — b7ce closed without writing it. | private |
| 2026-08-27 | 15:55 | 3385 | main | MT-382(e) fixed — six raw-error leak sites (not two) now show child-safe copy, with the shared error card made readable on dark bands; MT-409 shipped — abandoned generations are registered and rescued into the library at next launch. MT-410 filed for the device pass. | private |
| 2026-08-26 | 17:27 | f96a | main | **MT-408 closed and MT-381(a) fixed**: a story generation that outran the client's polling ceiling was re-submitted rather than resumed, so one slow story could be billed up to three times and then answered with canned fallback content while the real story sat finished server-side; the retry now resumes the existing task, at most once, with a fresh generation still available as the safety net. Separately, the footer arrows in the 11+ reader moved nothing because that layout renders every page into one scrolling list, which also kept the end-of-story rating row and Color chip out of reach for anyone who finished by scrolling. New: MT-409 (the tail case, where a story lands after the whole retry budget, is still unreachable). | private |
| 2026-08-25 | 18:34 | 5976 | main | Fixed and proved in production the server-side half of MT-408: a story that finished after its Celery worker died stayed unreachable because the status endpoint only consulted the persisted story row for one particular task state, so a stale in-flight state hid a completed story indefinitely. Verified by an A/B against production on one real generated story — old code reported still-working, new code returned the finished story, deployed commit the only variable. The client-side half (a generation finishing after the client stops polling) remains open on MT-408. | private |
| 2026-08-25 | 08:59 | 0062 | main | Fixed red CI on main (the lint gate also runs isort, which a black-only local check misses). Verified in production that the new AI cost telemetry writes real per-call rows (MT-407 closed), and fixed the result screen defaulting readers to age 7, which gave teens the kid flip-book — permanently on reopened saved stories (MT-381b). Filed MT-408: a backend deploy during generation loses the finished story and leaves the client polling forever. | private |
| 2026-08-24 | 22:04 | 17a7 | main | Cost telemetry shipped: every paid AI path now logs durable per-call costs, with budget alerting and a real admin cost report (MT-402 closed as delete-and-consolidate). Pick-a-Path now saves the whole traversed adventure, not just the final segment (MT-382a); build-stamp CI path verified in prod at open (MT-406 closed). | private |
| 2026-08-24 | 20:52 | 14bc | main | Empty close — no work this session; tree already in sync with origin. | private |
| 2026-08-24 | 17:03 | a032 | main | **Build identification**: a device can now name the build it is running — CI stamps the commit SHA into the web build, feeding both the Sentry release (previously always `1.0.0+1`, so no event could be attributed to a build) and a build stamp at the bottom of Parent Controls. Also confirmed the session-command project-deference guard works for both open and close, and verified production is serving the current bundle. | private |
| 2026-08-23 | 08:19 | aa0d | main | Consolidated the session commands to one open + one close; fixed MT-387 (onboarding asked for a name before it knew the user's age band, so every first-run user got the toddler screen) and made the splash skippable. | private |
| 2026-08-17 | 07:58 | a3c7 | main | Orientation; corrected a stale Apple-track status briefing | private |
