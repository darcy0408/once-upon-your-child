...
2. **COPPA enforcement intentionally OFF during testing (owner decision)** — the launch-gate flags were briefly flipped ON 2026-07-14, then the owner turned the enforcement flags back OFF ~2026-07-15 to remove test friction (the under-13 Resend consent round-trip forced a live email on every test). **Current prod state (verified 2026-07-17):** `COPPA_REQUIRE_VERIFIED_CONSENT` = `false`, `ENFORCE_RESOLVED_AGE` = `false`, `COPPA_REQUIRE_CURRENT_POLICY_VERSION` = `true`. **This is a deliberate pre-launch testing posture, not a bug — do NOT flip these ON without owner say-so.** All three must be set `true` before launch / before any untrusted tester gets an invite link; the flip is verified-safe (the returning-client landmines that forced earlier rollbacks — #442 age back-fill, #448 media-src CSP, #447 cache-buster, the Pages cutover — are all closed). Client-side, non-release builds already skip the email via `_kSkipEmailConsent`/`CONSENT_TEST_BYPASS`, so local testing needs no server flag change. Remaining on MT-310: the human sign-off read of `crisis_detection.py`/`input_sanitizer.dart`. (`ENCRYPTION_KEY` is set in prod — verified 2026-07-04, 07-14, and 07-17.)
