...
| ID | Status | Notes |
|----|--------|-------|
| PERF-01 | Backend done; UI consumer pending | Slices 1-4 implemented: `StoryGenerationService.generate_story_stream`; `_try_gemini` emits `partial_story:<task_id>` to Redis on each chunk; `/task-status` returns `partial_text` on PROCESSING/PENDING-with-partial; `generateStory(..., onPartial:)` callback plumbed in Dart. Slice 5 resolved 2026-05-31 as "Option 1: honest progress, no text" — the streamed partial_text is PRE-MODERATION, so it is NOT rendered; instead its length drives the `MagicalLoadingView` 4-dot indicator (replacing a blind 3.1s timer). On branch `perf-01-honest-progress` (commit 20141590), pending merge + `flutter analyze`/test. Rejected showing partial text to a child given the moderation ordering. |
...
| PERF-04 | Backend done; client wiring pending | `task_cancellation.py` helper + `POST /cancel-task/<task_id>` (auth + ownership) + `generate_story_task` now has 3 `is_cancelled` gates (task start, before each (re)generation, before moderation) — 2026-05-31. ARCHITECTURE CORRECTION: illustrations are a separate serial per-page endpoint (not a batch in the task) and the default provider is $0/image, so per-image cancellation is descoped — the story-text task is the real cost lever. PENDING: wire the wizard Cancel button to `cancelTask` (do on branch `perf-01-honest-progress`). See `perf-04-cancellation-design.md` |
